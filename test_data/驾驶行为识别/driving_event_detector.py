...
# ============================================================================
EVENT_TYPES = {
    # ── key 对齐 BEHAVIOR_LABELS_CN / MetadataRegistry ──
    # 速度基 (Speed-based)
    'constant_speed':        '匀速直行',
...
class DrivingEventDetector:
    ...
    def detect_speed_events(self):
        ...
        for i0, i1 in self._find_segments(
            ...
            if dur > 0.2 and ds > 1:
                self._add_event('normal_acceleration', self.t[i0], self.t[i1], 0.80,
        ...
        for i0, i1 in self._find_segments(
            ...
            ds = self.speed[i1] - self.speed[i0]
            if dur > 0.2 and ds < -1:
                self._add_event('normal_deceleration', self.t[i0], self.t[i1], 0.80,
                                {'speed_delta': float(ds), 'accel_mean': float(self.accel_ma[i0:i1].mean())})
    ...
    def detect_compound_events(self):
        ...
        for i0, i1 in self._find_segments(corner_accel, 0.3, 0.3):
            ds = self.speed[i1] - self.speed[i0]
            if ds > 1:
                self._add_event('cornering_acceleration', self.t[i0], self.t[i1], 0.75,
        ...
        for i0, i1 in self._find_segments(corner_decel, 0.3, 0.3):
            ds = self.speed[i1] - self.speed[i0]
            if ds < -1:
                self._add_event('cornering_deceleration', self.t[i0], self.t[i1], 0.75,
                                {'speed_delta': float(ds),
                                 'wheel_mean': float(self.wheel[i0:i1].mean())})
    ...
    def detect_aggressive_events(self):
        ...
        for i0, i1 in self._find_segments(
            ...
            self._add_event('aggressive_acceleration', self.t[i0], self.t[i1], 0.85,
        ...
        for i0, i1 in self._find_segments(
            ...
            self._add_event('aggressive_deceleration', self.t[i0], self.t[i1], 0.85,
        ...
        for i0, i1 in self._find_segments(
            ...
            self._add_event('emergency_braking', self.t[i0], self.t[i1], 0.90,
                            {'accel_min': float(self.vehicle_accel[i0:i1].min()),
    ...
    def detect_dynamic_events(self):
        ...
        for start in range(0, self.N - win, step):
            ...
            if (zero_crossings >= 2 and wheel_range > 5
                ...
                self._add_event('weaving', self.t[start], self.t[end], 0.75,
        ...
        for i0, i1 in self._find_segments(
            ...
            self._add_event('rapid_direction_change', self.t[i0], self.t[i1], 0.80,
    ...
    def detect_imu_events(self):
        ...
        for i0, i1 in self._find_segments(
            ...
            self._add_event('skid_risk', self.t[i0], self.t[i1], 0.75,
