...
class BatchTrainingDataGenerator:
    ...
    def discover_data_pairs(self) -> List[Tuple[str, str]]:
        ...
        Returns:
            [(parsed_csv_path, event_csv_path), ...]
        """
        pairs = []
        ...
        logger.info(f"发现 {len(parsed_files)} 个 parsed_data, {len(eval_dirs)} 个 expert_evaluation (含 event_analysis.csv)")
        ...
        for parsed_path in parsed_files:
            ...
            if best_eval:
                ...
                logger.info(f"配对: {os.path.basename(parsed_path)} ↔ {eval_name} (时间差={best_diff})")
            else:
